...
function calcNiNormStd(media, std, x){ // calcula los numeros Ni con la fórmula a+(b-a)*ri
...
function calcIntervals(intervalos,min,max,numIntervalos){// se calculan los intervalos 
    intervalos.push(min)
...
function countFrecuency(intervalos, ni, frecuencias){// se cuenta la cantidad de numeros dentro  de cada intervalo
    var aux = 0
...
function calcTotalChi2(chi2){// sumatoria de los chi2 por frecuencia
    var totalChi = 0
...
function calcChi2(frecObt,frecEsp,chi2,numIntervalos){ // calcula chi2 para cada frecuencia
    for(let i = 0; i < numIntervalos; i++){
        chi2.push(Math.pow(frecObt[i]-frecEsp[i],2)/frecEsp[i])
    }
}

function calcFrecEsp(frecEsp, numIntervalos, ni){// se calcula la frecuencia esperada n/numintervalos
    for(let i = 0; i < numIntervalos; i++){
        frecEsp.push(ni.length/numIntervalos)
    }
}

function calcFrecObt(start,end,ni,frecObt){// se cuenta la cantidad de numeros por intervalo
    var aux = 0
...
function calcLimitsIntervals(start,end,min,max,numIntervalos){//se calculan el inicio y fi nde cada intervalo
    start.push(min)
    for(let i = 0; i < numIntervalos; i++){
...
function calcNiNormal(a,b,ri,ni){// se calculan Ni con la formula a+(b-a)*ri
    for(let i = 0; i < ri.length; i++){
        ni.push(a+(b-a)*ri[i])
    }
}
